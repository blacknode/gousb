...
func (c *Context) GetPOSDevices(requireOpenDevices bool) (*POSDevices, error) {
	if c.ctx == nil {
		return nil, errors.New("invalid called with nil context")
	}
	devicesList, err := c.libusb.getDevices(c.ctx)
	if err != nil {
		return nil, err
	}
	var reterr error
	posDevices := &POSDevices{
		printers:       make([]*DeviceInfo, 0),
		barcodeReaders: make([]*DeviceInfo, 0),
	}
	for _, deviceOnList := range devicesList {
		deviceDescription, err := c.libusb.getDeviceDesc(deviceOnList)
		defer c.libusb.dereference(deviceOnList)
		if err != nil {
			reterr = err
			continue
		}

		if deviceDescription.Class != ClassPrinter &&
			deviceDescription.Class != ClassHID {
			continue
		}

		deviceHandler, err := c.libusb.open(deviceOnList)
		if err != nil {
			reterr = err
			continue
		}

		device := &Device{handle: deviceHandler, ctx: c, Desc: deviceDescription}
		productName, _ := device.Product()
		vendorName, _ := device.Manufacturer()
		if !requireOpenDevices {
			device.Close()
		}
		if deviceDescription.Class == ClassPrinter {
			posDevices.printers = append(posDevices.printers, &DeviceInfo{
				ProductID:      deviceDescription.Product,
				VendorID:       deviceDescription.Vendor,
				VendorName:     vendorName,
				ProductName:    productName,
				AttachedDevice: device,
				DeviceIsOpen:   device.IsOpen(),
			})
		}
		if deviceDescription.Class == ClassHID {
			posDevices.barcodeReaders = append(posDevices.barcodeReaders, &DeviceInfo{
				ProductID:      deviceDescription.Product,
				VendorID:       deviceDescription.Vendor,
				VendorName:     vendorName,
				ProductName:    productName,
